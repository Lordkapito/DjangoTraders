...
under a prefix.
  ...
  4. Added product_detail_url
        Same shape as customer_detail_url, but with an <int:...>
        converter -- Product.product_id is a SmallIntegerField PK, not
        a CharField like Customer.customer_id, so str would accept
        values that could never match a row. No longer an assignment
        component now that views.product_detail exists.
"""
