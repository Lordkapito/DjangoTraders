...
class Customer(models.Model):
    ...
    def search(cls, company_name="", contact_name="", city="", contact_title="", country=""):
        ...
        name, city, contact title, and country, returning every customer
        when none are given. Used by customer_list (djtraders/views.py)
        to back its five-field search form.
        ...
        company_name, contact_name, and city all use "icontains" (a
        case-insensitive substring match) since they're free-text fields
        -- someone typing part of a value expects a match anywhere in
        it, regardless of case. contact_title and country use an exact
        match instead, since both are populated from dropdowns built off
        values already on record, not free text -- there's nothing
        partial (or case-varied) to match against.
        """
...
class Product(models.Model):
    ...
    def search(cls, product_name="", category_id="", supplier_id="", show_all=False):
        ...
        supplier_id uses an exact match, since its value comes from a
        dropdown of suppliers already on record, not free text.
        ...
        # if there is a supplier_id, filter by it;
        if supplier_id:
